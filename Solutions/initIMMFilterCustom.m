function filter = initIMMFilterCustom(detection)
% initIMMFilterCustom  Initialize an IMM filter with trackingEKFCustom filters

filter1 = initcvekf(detection);
filter2 = initctekf(detection);

% Convert each filter to trackingEKFCustom
filter1 = convertToCustom(filter1);
filter2 = convertToCustom(filter2);
...
function customFilter = convertToCustom(ekfFilter)
    customFilter = trackingEKFCustom( ...
        'StateTransitionFcn', ekfFilter.StateTransitionFcn, ...
        'StateTransitionJacobianFcn', ekfFilter.StateTransitionJacobianFcn, ...
        'MeasurementFcn', ekfFilter.MeasurementFcn, ...
        'MeasurementJacobianFcn', ekfFilter.MeasurementJacobianFcn, ...
        'HasAdditiveProcessNoise', ekfFilter.HasAdditiveProcessNoise, ...
        'HasAdditiveMeasurementNoise', ekfFilter.HasAdditiveMeasurementNoise, ...
        'State', ekfFilter.State, ...
        'StateCovariance', ekfFilter.StateCovariance, ...
        'ProcessNoise', ekfFilter.ProcessNoise, ...
        'MeasurementNoise', ekfFilter.MeasurementNoise);
end
